// Wait for 4 seconds before executing the code
setTimeout(() => {
  ...
  if (tableElements.length > 1) {
    const table2 = tableElements[1]; // Access the 2nd table (0-indexed)
    const tbody = table2.querySelector("tbody");
    if (!tbody) {
      console.error("No tbody found in the 2nd table.");
      return;
    }
    ...
    if (rows.length > 2) {
      // Extract the name from the 3rd row and 2nd <td>
      const row3 = rows[2]; // Access the 3rd row
      const secondTd3 = row3.querySelectorAll("td")[1]; // Access the 2nd <td> (0-indexed)
      if (secondTd3) {
      ...
      console.error("Not enough rows in the 2nd table.");
    }
  } else {
    console.error("2nd table not found.");
  }

  // Existing code for other data extraction
  ...
  if (rows7.length > 8) {
    // Extract text after "Título:" from the 7th row's second <td>
    const row7 = rows7[6];
    const secondTd7 = row7.querySelectorAll("td")[1];
    const title = secondTd7.textContent.trim();
    console.log("Title extracted:", title);

    // Extract date and period from the 9th row
    const row9 = rows7[8];
    ...
    if (secondTd9) {
      ...
      pElements9.forEach(p => {
        ...
        // Look for a <p> that contains "Dia: " and extract the date
        ...
        // Look for a <p> that contains "Período: " and extract the period
    ...
    // Try to find the <td> element with colspan="2"
    ...
    if (tdElement && boldElement && tdElement1.length > 3) {
      ...
      const cityMatch = cityText.match(/^(\w+)/);
      ...
      if (boldElement && boldElement.textContent.includes("Informação cadastral")) {
        ...
        if (match && match[1]) {
          ...
          // Fetch for city, cadastro_number, date, period, and title
          fetch('http://127.0.0.1:50000', {
            ...
            body: JSON.stringify({
              ...
              period: period,
              title: title,
              name: name // Send the extracted name
            }),
          })
          .then(response => response.json())
          .then(data => {
            console.log("Server response for city, cadastro_number, date, period, title, and name:", data);
          })
          .catch(error => {
